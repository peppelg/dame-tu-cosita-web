...
function $MenuBar(menus){
	...
	function $MenuPopup(menu_items){
		var $menu_popup = $("<div>").addClass("menu-popup");
		var $menu_popup_table = $("<table>").addClass("menu-popup-table").appendTo($menu_popup);
		
		$.map(menu_items, function(item){
			var $row = $("<tr>").addClass("menu-row").appendTo($menu_popup_table)
			if(item === $MenuBar.DIVIDER){
				var $td = $("<td>").attr({colspan: 4}).appendTo($row);
				var $hr = $("<hr>").addClass("menu-hr").appendTo($td);
			}else{
				var $item = $row.addClass("menu-item");
				var $checkbox_area = $("<td>").addClass("menu-item-checkbox-area");
				var $label = $("<td>").addClass("menu-item-label");
				var $shortcut = $("<td>").addClass("menu-item-shortcut");
				var $submenu_area = $("<td>").addClass("menu-item-submenu-area");
	...
	$.each(menus, function(menus_key, menu_items){
		var $menu_container = $("<div>").addClass("menu-container").appendTo($menus);
